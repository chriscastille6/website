...
learning_correlations <- data.frame(
  ...
  variable1 = c(
    # Intuitive examples
    "Height", "Age", "Nearness to the equator", 
    "Gender (female vs male)", "Education level",
  ...
  variable2 = c(
    # Intuitive examples
    "Weight", "Income", "Daily temperature",
...
server <- function(input, output, session) {
  ...
  save_to_database <- function(session_id, question_data) {
    tryCatch({
      con <- dbConnect(SQLite(), db_path)
      
      # Save session if it doesn't exist
      session_exists <- dbGetQuery(con, "SELECT COUNT(*) as count FROM user_sessions WHERE session_id = ?", list(session_id))
      if (session_exists$count == 0) {
        dbExecute(con, "INSERT INTO user_sessions (session_id, consent_given) VALUES (?, ?)", 
                 list(session_id, game_state$consent_given))
      }
    ...
    if (!is.null(input$user_correlation)) {
      ...
      if (game_state$consent_given) {
        question_data <- list(
          ...
          variable1 = game_state$current_variable1,
          variable2 = game_state$current_variable2,
          user_guess = user_correlation_guess,
          correct_answer = correct_correlation,
          error = error
        )
        save_to_database(game_state$session_id, question_data)
      }
